...
def extract_timestamps(app_log_file):
    ...
    with open(app_log_file, 'r') as f:
        for line in f:
            ...
            if match:
                timestamps.add(match.group(0)[:19])
    return timestamps
...
def main():
    ...
    for app_log_filename in os.listdir(app_log_directory):
        if app_log_filename.startswith('app_') and app_log_filename.endswith('.pcap.log'):
            ...
            pod_specific_name = app_log_filename.split('app_')[1].replace('.pcap.log', '')  # 'teastore-webui-59ccf6c8b5-b6lfb'
            ...
            # Check if pre_cl_log_file exists before filtering
